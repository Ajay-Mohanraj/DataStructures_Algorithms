...
public class TestFordFrivolous2_Driving extends BCATestScenario {
	...
	public int runTest() {
		...
		f2.drive(30);
		assertEquals(f2.getMileage(), 31, 0.1, "Mileage should be 31.");
		f2.drive(200);
		assertEquals(f2.getMileage(), 231, 0.1, "Mileage should be 231.");
		assertEquals(f2.getRemainingRange(), 242, 0.1, "Remaining range should be 242.");
		...
		f2.drive(242);
		assertEquals(f2.getMileage(), 473, 0.1, "Mileage should be 473.");
		...
		f2.refillTank();
		assertEquals(f2.getFuelLevel(), f2.getFuelCapacity(), 0.1, "Fuel level should be 20 after refilling.");
