...
public class HuffViewer {
	...
	private void compress(File original, StatusBar status, HuffPanel panel) {
		...
		try {
			...
			Thread thread = new Thread(() -> {
				...
				try {
					Processor processor = new HuffProcessor();
	...
	private void test(File directory, boolean useHF, StatusBar status, HuffPanel panel) {
		...
		List<File> toDecompress = new ArrayList<>();
		for (File file : files) {
		...
		Thread thread = new Thread(() -> {
			...
			File[] uncompressed = new File[compressed.length];
			for (int i = 0; i < compressed.length; i++) {
				File file = compressed[i];
				try {
					updater.updateStatus(Status.Working, "decompressing " + file.getName());
					BitInputStream in = new BitInputStream(file);
					uncompressed[i] = new File(file.getPath() + ".unhf");
					BitOutputStream out = new BitOutputStream(uncompressed[i]);
					updater.setGenerator(() -> {
						return (updater.progress() + in.getBitsRead()) / (8.0 * sum);
					});
					Processor processor = new HuffProcessor();
					...
					System.err.println("problem uncompressing " + file.getName());
				}
			}
			updater.stop();
